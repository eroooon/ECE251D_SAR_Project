function data = helperBackProjection(sigdata,rnggrid,fastTime,fc,fs,...
                                     prf,speed,crossRangeResolution,c)
% Back-Projection Algorithm
% This function demonstrates the time-domain back projection algorithm for
% imaging the side-looking synthetic aperture radar. The pulsed compressed
% synthetic aperture data is taken as input in this algorithm. Initialize
% the output matrix.

data = zeros(size(sigdata));
azimuthDist = -200:speed/prf:200;%azimuth distance

% Limit the range and cross-range pixels being processed to reduce
% processing time.
rangelims = [700 1400];
